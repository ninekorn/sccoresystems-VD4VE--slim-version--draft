                ...
                for (int y = -ChunkHeight_L; y <= ChunkHeight_R; y++)
                {
                    for (int z = -ChunkDepth_L; z <= ChunkDepth_R; z++)
                    {
                        ...
                        else if (distance >= ((ChunkWidth_L) * 0.35f) && distance < ((ChunkWidth_L) * 0.975f)) //0.35f 0.45f
                        {
                            ...
                            int randX = (int)(Math.Round(sc_maths.getSomeRandNumThousandDecimal((int)Math.Round(minX), (int)Math.Round(maxX), _decimal_for_random, 0)));
                            //int maxDist_height = (int)(Math.Round((ChunkHeight_L + ChunkHeight_R + 1) * max_spike_length_for_random));
                            int randY = (int)(Math.Round(sc_maths.getSomeRandNumThousandDecimal((int)Math.Round(minY), (int)Math.Round(maxY), _decimal_for_random, 0)));
                            //int maxDist_depth = (int)(Math.Round((ChunkDepth_L + ChunkDepth_R + 1) * max_spike_length_for_random));
                            int randZ = (int)(Math.Round(sc_maths.getSomeRandNumThousandDecimal((int)Math.Round(minZ), (int)Math.Round(maxZ), _decimal_for_random, 0)));
